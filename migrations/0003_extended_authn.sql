...
CREATE UNIQUE INDEX totp_factors_user_id_idx ON auth.totp_factors (user_id)
...
CREATE TABLE auth.totp_recovery_codes (
    id        uuid        NOT NULL PRIMARY KEY DEFAULT uuidv7()
                          CHECK (auth.uuid_version(id) = 7),
    factor_id uuid        NOT NULL REFERENCES auth.totp_factors(id) ON DELETE CASCADE,
...
CREATE INDEX totp_recovery_codes_factor_id_idx ON auth.totp_recovery_codes (factor_id)
...
CREATE UNIQUE INDEX passkey_credentials_credential_id_idx
    ON auth.passkey_credentials (credential_id)
    INCLUDE (id, user_id)
    WHERE deleted_at IS NULL;
...
-- Auth index mirrors tokens_auth_idx: validates id+secret, returns kind+user_id+context+expires_at
-- without a heap fetch for both the DELETE...RETURNING and the fallback SELECT.
CREATE INDEX one_time_tokens_auth_idx ON auth.one_time_tokens (id, secret)
    INCLUDE (kind, user_id, context, expires_at);

-- GC index: sweep expired tokens without a seq scan.
CREATE INDEX one_time_tokens_expires_at_idx ON auth.one_time_tokens (expires_at);
