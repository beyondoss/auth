...
pub fn compile(schema: &AuthzSchema) -> Result<CompiledSchema, SchemaError> {
    ...
    let resource_map: HashMap<&str, &ResourceDef> = schema
        .resources
        .iter()
...
impl CompiledSchema {
    ...
    pub fn build_or_chain(&self, resource_type: &str, permission: &str) -> Option<String> {
        ...
        Some(
            calls
                .iter()
                .map(|c| format_call(c, "subject.subject_id", "$3"))
                .collect::<Vec<_>>()
                .join("\n    OR "),
        )
    }
    ...
    pub fn build_batch_or_chain(&self, resource_type: &str, permission: &str) -> Option<String> {
        ...
        Some(
            calls
                .iter()
                .map(|c| format_call(c, "$1", "t.object_id"))
                .collect::<Vec<_>>()
                .join("\n    OR "),
        )
    }
    ...
    match call {
        ...
        AuthzCheckCall::MultiHop {
            ...
            let terms = terminal_relations
                ...
                .join(", ");
            format!(
                "auth.authz_check_path({subject}, ARRAY[{prefix}]::text[], ARRAY[{types}]::text[], ARRAY[{terms}]::text[], {object})"
            )
        }
    }
}
